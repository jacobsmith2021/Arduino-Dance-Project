...
To Do
  ...
  2) fix bug where receiver takes a long time to read the current send data
    (backlog issue?)
*/
//values to change
...
void setup(void) {
  ...
  if (SEND_MODE){
    ...
    x_zero=mpu.getAngleX();      //this will be used to zero the IMU based on its initial position
    y_zero=mpu.getAngleY();
...
void loop() {
  ...
  if (SEND_MODE){
    ...
    mpu.update();  
// IMU is oriented Horizontal, face up
      // y_axis_value=mpu.getAngleY();
      //  x_axis_value=mpu.getAngleX();
y_axis_value=mpu.getAngleY()-y_zero;
x_axis_value=mpu.getAngleX()-x_zero;

// IMU is oriented Horiztonally face down
   // x_initial=mpu.getAngleX();
   // if (x_initial<-1){
   //   x_axis_value=-1*(x_initial+180);
   // } else {
   //   x_axis_value=180-x_initial;
   // }
   // y_axis_value=mpu.getAngleY();

// IMU is oriented vertically facing away from you
   // x_axis_value=mpu.getAngleX()-90;
   // y_axis_value=mpu.getAngleY();
